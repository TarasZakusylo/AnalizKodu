...
//     	Redactor.add(t_rekvizutu);     	
	...
	public class eHandler implements ActionListener {
			...
			try {
				if (e.getSource() == b_perevirutu) {
					...
					if (ret == JFileChooser.APPROVE_OPTION) {
						...
						File filesPath = new java.io.File(ch);
						...
						for (int i = 0; i < filesList.length; i++) {
							...
							if (buf.endsWith(".flv") || buf.endsWith(".swf") || buf.endsWith(".flash") || buf.endsWith(".applet")) {
								 mitka = 1;
								if( k == 0 ){
						...
						if( mitka == 1 ) JOptionPane.showMessageDialog(null, " \""+ nebezpeka + "\" є підозрілим, рекомендовано проаналізувати.");
						if( mitka1 == 1 )JOptionPane.showMessageDialog(null , " \""+ nebezpeka1+ "\" є підозрілим, рекомендовано проаналізувати.");
						if( mitka2 == 1 )JOptionPane.showMessageDialog(null , " \""+ nebezpeka2+ "\" є підозрілим, рекомендовано проаналізувати.");

						if( mitka == 0 && mitka1 == 0  && mitka2 == 0)JOptionPane.showMessageDialog(null , "Безпечно");
		...
		private void deleteIfExists(String buf) {
